...
This interactive document was created using `learnr`. If you would like to follow along on your own computer, you can download this file from my website: 

[jeswheel.github.io](https://jeswheel.github.io).

Alternatively, you can clone the public GitHub repository that contains this document (jeswheel/ISU_ttest): 

```{git, echo=TRUE, eval=FALSE}
git clone https://github.com/jeswheel/ISU_ttest.git
```
...
### Some examples: 
  ...
  - TODO: Change this. It's more of proportions than means. In the 1950's, Researchers want to learn about the effectiveness of polio vaccines. They randomly assigned students from the same schools and ages to control and treatment groups. Children in the control group receive a placebo, and those in the control group receive a polio vaccine. 
  After a few years, data were collected to evaluate the total number of polio infections in each group. 
     - The response variable for each group $X_i$ is total number of polio cases.
...
<!-- An economist wishes to relate the speed with which a particular insurance innovation is adopted ($Y$ , in months) to the size of the insurance firm ($X1$, in millions of dollars) and the type of firm ($X2$, either mutual (0) or stock firms (1)). -->
