...
or 
$$
H_1: \,\,\, \mu_1 - \mu_2 > 0\,\,\,\,\,\,\,\,\,\,\text{or}\,\,\,\,\,\,\,\,\,\,H_1: \,\,\, \mu_1 - \mu_2 < 0
$$
...
```{r echo=FALSE, fig.height=2.75, fig.width=4.25, fig.align='center'}
...
ys <- dt(x = xs, df = 15)
# Plot the curve
curve(dt(x, df = 15), from = -5, to = 5, main = "t-distribution with DF=15", xlab = '', ylab = '')
# Plot the shading
...
kable(table_content, format = "html", escape = FALSE, table.attr = "style='width:100%'",col.names = NULL) 
# %>% 
  # kable_styling()
```
...
```{r, echo=FALSE, fig.height=2.75, fig.width=4,fig.align='center'}
chickens %>% 
...
```{r, fig.height=2.75, fig.width=3.5, fig.align='center'}
# Calculate the critical value to which you want to plot
...
ys2 <- dt(x = xs2, df = df)
# Plot the curve
curve(dt(x, df = df), from = -5, to = 5, main = paste0("t-distribution with df = ", df), xlab = '', ylab = '')
# Plot the shading
